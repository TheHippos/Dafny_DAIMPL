// Dafny program Sudoku.dfy compiled into C#
...
module SudokuSolver {
    ...
    decreases board
  {
    ...
    var hasOnlyValidDigits := hasOnlyValidDigits(board);
  ...
  method copy(board: Board) returns (boardCopy: Board)
    requires is9x9(board)
    ensures is9x9(boardCopy)
    ensures forall r: uint8, c: uint8 {:trigger BoardsEqualFrom(board, boardCopy, r, c)} {:trigger BoardsEqualUpTo(board, boardCopy, r, c)} {:trigger boardCopy[r, c]} {:trigger board[r, c]} :: (0 <= r < 9 && 0 <= c < 9 ==> board[r, c] == boardCopy[r, c]) && (0 <= r < 9 && 0 <= c < 9 ==> BoardsEqualUpTo(board, boardCopy, r, c)) && (0 <= r < 9 && 0 <= c < 9 ==> BoardsEqualFrom(board, boardCopy, r, c))
    ensures EmptySlotCount(board) == EmptySlotCount(boardCopy)
    ensures board != boardCopy
    decreases board
  {
  ...
  predicate BoardsEqualUpTo(b1: Board, b2: Board, r: uint8, c: uint8)
    ...
    reads b1, b2
    decreases {b1, b2}, b1, b2, r, c
  {
    forall i: uint8, j: uint8 {:trigger b2[i, j]} {:trigger b1[i, j]} :: 
      (i < r || (i == r && j <= c)) &&
      0 <= i < 9 &&
      0 <= j < 9 ==>
        b1[i, j] == b2[i, j]
  ...
  lemma /*{:_inductionTrigger EmptySlotCountRecursiveDownwards(b2, r, c), EmptySlotCountRecursiveDownwards(b1, r, c)}*/ /*{:_inductionTrigger EmptySlotCountRecursiveDownwards(b2, r, c), is9x9(b1)}*/ /*{:_inductionTrigger EmptySlotCountRecursiveDownwards(b1, r, c), is9x9(b2)}*/ /*{:_inductionTrigger BoardsEqualUpTo(b1, b2, r, c)}*/ /*{:_induction b1, b2, r, c}*/ lemma_EmptySlotCountRecursiveSameUpTo(b1: Board, b2: Board, r: uint8, c: uint8)
    requires is9x9(b1) && is9x9(b2)
    requires 0 <= r < 9 && 0 <= c < 9
    ...
    decreases 9 * r + c
  {
    if r == 0 && c == 0 {
      ...
      ghost var prev_r := prevRow(r, c);
      ghost var prev_c := prevColumn(r, c);
      assert BoardsEqualUpTo(b1, b2, prev_r, prev_c);
      lemma_EmptySlotCountRecursiveSameUpTo(b1, b2, prev_r, prev_c);
    }
  }

  lemma /*{:_inductionTrigger EmptySlotCountRecursiveUpwards(b2, r, c), EmptySlotCountRecursiveUpwards(b1, r, c)}*/ /*{:_inductionTrigger EmptySlotCountRecursiveUpwards(b2, r, c), is9x9(b1)}*/ /*{:_inductionTrigger EmptySlotCountRecursiveUpwards(b1, r, c), is9x9(b2)}*/ /*{:_inductionTrigger BoardsEqualFrom(b1, b2, r, c)}*/ /*{:_induction b1, b2, r, c}*/ lemma_EmptySlotCountRecursiveSameFrom(b1: Board, b2: Board, r: uint8, c: uint8)
    requires is9x9(b1) && is9x9(b2)
    requires 0 <= r < 9 && 0 <= c < 9
    ...
    decreases 81 - (9 * r + c)
  {
    if r == 8 && c == 8 {
      ...
      ghost var next_r := nextRow(r, c);
      ghost var next_c := nextColumn(r, c);
  ...
  lemma /*{:_inductionTrigger EmptySlotCountRecursiveUpwards(board, nextRow(r, c), nextColumn(r, c))}*/ /*{:_inductionTrigger board[r, c]}*/ /*{:_inductionTrigger EmptySlotCountRecursiveDownwards(board, prevRow(r, c), prevColumn(r, c))}*/ /*{:_inductionTrigger EmptySlotCount(board)}*/ /*{:_inductionTrigger is9x9(board)}*/ /*{:_induction board}*/ lemma_emptyCountIsSplitByCell(board: Board, r: uint8, c: uint8)
    requires is9x9(board)
    requires 0 <= r < 9 && 0 <= c < 9
    ensures EmptySlotCount(board) == (if r == 0 && c == 0 then 0 else EmptySlotCountRecursiveDownwards(board, prevRow(r, c), prevColumn(r, c))) + (if board[r, c] == 0 then 1 else 0) + if r == 8 && c == 8 then 0 else EmptySlotCountRecursiveUpwards(board, nextRow(r, c), nextColumn(r, c))
    decreases board, r, c
  {
  ...
  lemma lemma_updatingBoardIncreasesEmptyCount(board: Board, oldBoard: Board, r: uint8, c: uint8)
    ...
    requires board[r, c] == 0
    requires EmptySlotCount(board) > 0
    requires forall i: uint8, j: uint8 {:trigger board[i, j]} {:trigger oldBoard[i, j]} :: 0 <= i < 9 && 0 <= j < 9 ==> (i == r && j == c && oldBoard[i, j] != 0 && board[i, j] == 0) || board[i, j] == oldBoard[i, j]
    ensures EmptySlotCount(board) == EmptySlotCount(oldBoard) + 1
    decreases board, oldBoard, r, c
  {
    lemma_emptyCountIsSplitByCell(oldBoard, r, c);
    ghost var count_before_old := if r == 0 && c == 0 then 0 else EmptySlotCountRecursiveDownwards(oldBoard, prevRow(r, c), prevColumn(r, c));
    ghost var count_after_old := if r == 8 && c == 8 then 0 else EmptySlotCountRecursiveUpwards(oldBoard, nextRow(r, c), nextColumn(r, c));
    lemma_emptyCountIsSplitByCell(board, r, c);
    ghost var count_before_new := if r == 0 && c == 0 then 0 else EmptySlotCountRecursiveDownwards(board, prevRow(r, c), prevColumn(r, c));
    ghost var count_after_new := if r == 8 && c == 8 then 0 else EmptySlotCountRecursiveUpwards(board, nextRow(r, c), nextColumn(r, c));
    if r > 0 || c > 0 {
      ghost var prev_r := prevRow(r, c);
      ghost var prev_c := prevColumn(r, c);
  ...
  lemma lemma_updatingBoardReducesEmptyCount(board: Board, oldBoard: Board, r: uint8, c: uint8, digit: sValue)
    ...
    requires digit != 0
    requires forall i: uint8, j: uint8 {:trigger board[i, j]} {:trigger oldBoard[i, j]} :: 0 <= i < 9 && 0 <= j < 9 ==> (i == r && j == c && oldBoard[i, j] == 0 && board[i, j] == digit) || board[i, j] == oldBoard[i, j]
    ensures EmptySlotCount(board) == EmptySlotCount(oldBoard) - 1
    decreases board, oldBoard, r, c, digit
  {
    lemma_emptyCountIsSplitByCell(oldBoard, r, c);
    ghost var count_before_old := if r == 0 && c == 0 then 0 else EmptySlotCountRecursiveDownwards(oldBoard, prevRow(r, c), prevColumn(r, c));
    ghost var count_after_old := if r == 8 && c == 8 then 0 else EmptySlotCountRecursiveUpwards(oldBoard, nextRow(r, c), nextColumn(r, c));
  ...
  method FindEmptySlot(board: Board) returns (slot: Option<(uint8, uint8)>)
    requires is9x9(board)
    ensures match slot case Some(coords) => 0 <= coords.0 < 9 && 0 <= coords.1 < 9 && board[coords.0, coords.1] == 0 && EmptySlotCount(board) > 0 case None() => forall r: uint8, c: uint8 {:trigger board[r, c]} :: (0 <= r < 9 && 0 <= c < 9 ==> board[r, c] != 0) && (0 <= r < 9 && 0 <= c < 9 ==> EmptySlotCount(board) == 0)
    decreases board
  {
    for i: uint8 := 0 to 9
      invariant forall r: uint8, c: uint8 {:trigger board[r, c]} :: 0 <= r < i && 0 <= c < 9 ==> board[r, c] != 0
      invariant i > 0 ==> EmptySlotCountRecursiveDownwards(board, i - 1, 8) == 0
    {
      for j: uint8 := 0 to 9
        invariant forall c: uint8 {:trigger board[i, c]} :: 0 <= c < j ==> board[i, c] != 0
  ...
  function EmptySlotCountRecursiveUpwards(board: Board, r: uint8, c: uint8): (count: uint8)
    ...
    ensures count == 0 <==> forall i: uint8, j: uint8 {:trigger board[i, j]} :: (i > r || (i == r && j >= c)) && 0 <= i < 9 && 0 <= j < 9 ==> board[i, j] != 0
    ...
    decreases (board.Length0 * board.Length1) as uint8 - (board.Length1 as uint8 * r + c)
  {
    ...
    if r == 8 && c == 8 then
      ...
      var nextRow: uint8 := nextRow(r, c); var nextColumn: uint8 := nextColumn(r, c); var recursiveResult: uint8 := EmptySlotCountRecursiveUpwards(board, nextRow, nextColumn); current_cell_val + recursiveResult
  }
    ...
    decreases 9 * r + c
  {
    ...
    if r == 0 && c == 0 then
      ...
      var prev_r: uint8 := prevRow(r, c); var prev_c: uint8 := prevColumn(r, c); var recursiveResult: uint8 := EmptySlotCountRecursiveDownwards(board, prev_r, prev_c); current_cell_val + recursiveResult
  }
  ...
  function EmptySlotCount(board: Board): (count: uint8)
    ...
    ensures count == 0 <==> forall r: uint8, c: uint8 {:trigger board[r, c]} :: 0 <= r < 9 && 0 <= c < 9 ==> board[r, c] != 0
    ensures count > 0 <==> exists r: uint8, c: uint8 {:trigger board[r, c]} :: 0 <= r < 9 && 0 <= c < 9 && board[r, c] == 0
  ...
  lemma /*{:_inductionTrigger EmptySlotCountRecursiveUpwards(board, 0, 0)}*/ /*{:_inductionTrigger EmptySlotCountRecursiveDownwards(board, 8, 8)}*/ /*{:_inductionTrigger is9x9(board)}*/ /*{:_induction board}*/ lemma_upwardsAndDownwardsCountingSame(board: Board)
    requires is9x9(board)
  ...
  lemma /*{:_inductionTrigger nextColumn(r, c), EmptySlotCountRecursiveUpwards(board, 0, 0)}*/ /*{:_inductionTrigger nextColumn(r, c), is9x9(board)}*/ /*{:_inductionTrigger nextRow(r, c), EmptySlotCountRecursiveUpwards(board, 0, 0)}*/ /*{:_inductionTrigger nextRow(r, c), is9x9(board)}*/ /*{:_inductionTrigger EmptySlotCountRecursiveDownwards(board, r, c)}*/ /*{:_induction board, r, c}*/ lemma_count_split(board: Board, r: uint8, c: uint8)
    requires is9x9(board)
    requires 0 <= r < 9 && 0 <= c < 9
    ensures EmptySlotCountRecursiveUpwards(board, 0, 0) == EmptySlotCountRecursiveDownwards(board, r, c) + if r == 8 && c == 8 then 0 else EmptySlotCountRecursiveUpwards(board, nextRow(r, c), nextColumn(r, c))
    decreases 9 * r + c
  {
    if r == 0 && c == 0 {
    } else {
    ...
    decreases {board}, board
  {
    forall r: uint8, c: uint8 {:trigger board[r, c]} :: 
      0 <= r < 9 &&
      0 <= c < 9 ==>
        isValidDigit(board, r, c, board[r, c])
  }
...
namespace Datatypes {
  ...
  public abstract class Option<T> : _IOption<T> {
    ...
    public static Datatypes._IOption<T> Default() {
      return create_None();
    }
    public static Dafny.TypeDescriptor<Datatypes._IOption<T>> _TypeDescriptor() {
      return new Dafny.TypeDescriptor<Datatypes._IOption<T>>(Datatypes.Option<T>.Default());
    }
  ...
  public class Option_None<T> : Option<T> {
    ...
    public override bool Equals(object other) {
      var oth = other as Datatypes.Option_None<T>;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
    ...
    public override string ToString() {
      string s = "Datatypes.Option.None";
      return s;
    }
  }
  public class Option_Some<T> : Option<T> {
    ...
    public override bool Equals(object other) {
      var oth = other as Datatypes.Option_Some<T>;
    ...
    public override string ToString() {
      string s = "Datatypes.Option.Some";
      s += "(";
...
namespace SudokuSolver {
  ...
  public partial class __default {
    ...
    public static Datatypes._IOption<byte[,]> Solve(byte[,] board)
    {
      ...
      _0_hasOnlyValidDigits = SudokuSolver.__default.hasOnlyValidDigits(board);
      if (!(_0_hasOnlyValidDigits)) {
        result = Datatypes.Option<byte[,]>.create_None();
        return result;
      }
      bool _1_isValid;
      _1_isValid = SudokuSolver.__default.isValidBoard(board);
      if (!(_1_isValid)) {
        result = Datatypes.Option<byte[,]>.create_None();
        return result;
      }
